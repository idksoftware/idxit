...
static inline uint64_t crc64_little(uint64_t crc, unsigned char* buf, size_t len)
{
...
static inline uint64_t crc64_big(uint64_t crc, unsigned char* buf, size_t len)
{
    unsigned char* next = buf;
...
uint64_t CRC64::crc64(uint64_t crc, unsigned char* buf, size_t len)
{
    uint64_t n = 1;
...
static uint64_t update_crc(uint64_t crc, FILE* fp)
{
    unsigned char cbuf;
    //uint64_t c = crc;
    while (feof(fp)) {
    ...
    do {
        //printf("%c\n", cbuf);
        crc = crc64_little_table[0][(crc ^ cbuf) & 0xff] ^ (crc >> 8);
        fread(&cbuf, 1, 1, fp);
        if (ferror(fp)) {
            return false;
        }
...
uint64_t CRC64::crc64(FILE* fp)
{
    return update_crc(0xffffffffffffffffL, fp) ^ 0xffffffffffffffffL;
};

uint64_t CRC64::crc64(const char* szPath, bool *status)
{
    *status = true;
    uint64_t count = 0;
    CRC64 l_crc;
    ...
    if (!fp)
    {
        *status = false; // File may be in use
        return 0;
    }

    count = l_crc.crc64(fp);
    if (fclose(fp))
    {
        *status = false;
        return 0;
