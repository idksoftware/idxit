...
bool SetEnv(const char *szSIAHome, bool all)
{
	...
	if (all) {
		std::string regPath = "SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment";
		result = RegCreateKeyEx(HKEY_LOCAL_MACHINE, (LPCWSTR)regPath.c_str(),
										0, NULL, 0, KEY_WRITE, NULL, &hkey, &dwDisposition);
	}
	else {
		std::string regPath = "Environment";
		result = RegCreateKeyEx(HKEY_CURRENT_USER, (LPCWSTR)regPath.c_str(),
										0, NULL, 0, KEY_WRITE, NULL, &hkey, &dwDisposition);
	}
    ...
    if(result == ERROR_SUCCESS)
    {
        ...
        LONG setResult = RegSetValueEx(hkey, TEXT("IDXIT_HOME"), 0, dwType,
...
bool GetEnv(const char *szSIAHome, std::string &resultStr, bool all)
{
	...
	result = RegGetValue(HKEY_LOCAL_MACHINE, regPath, "IDXIT_HOME", NULL, NULL, NULL, NULL);
